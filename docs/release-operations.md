...
After the first published fork nightly, the saved URL, ETag, Last-Modified value, content length, and SHA-256 identify both processed upstream APKs. Each heartbeat performs inexpensive HEAD checks against the last processed pair before selecting a new one. If Libretro later rebuilds that dated pair, the monitor reports the changed fields but preserves the immutable published release provenance and continues with newer dates. A historical release is never silently replaced or republished; all new candidate APKs still undergo the complete download, hash, revision, signer, package, ABI, and provenance verification gates.
...
4. If `/storage/emulated/0/RetroArch/retroarch.cfg` exists, preserve it and review the difference manually. Otherwise copy the active config there. This is a one-time manual signer-transition operation; the fork performs no runtime copying or synchronization.
